...
def extract_backbone(graph, weights, alpha):
    ...
    for n in graph:
        k_n = len(graph[n])
        if k_n > 1:
            sum_w = sum(weights[n, nj] for nj in graph[n])
            for nj in graph[n]:
...
def main():
    ...
    df_category = pd.merge(df, ingr, left_on='ingr1',
                           right_on='ingredient name').drop('ingredient name',
                                                            axis=1)
    ...
    for i in range(df_subset.shape[0]):
        ...
        weights[df_subset.iloc[i, 1],
                df_subset.iloc[i, 0]] = df_subset.iloc[i, 2]

    alpha = 0.04
    graph1_backbone = extract_backbone(graph1, weights, alpha)

    # make weights into a list and include both directions
    weights_subset = []
    for (u, v) in graph1_backbone.edges():
        weights_subset.append((u, v, weights[u, v]))
        weights_subset.append((v, u, weights[u, v]))

    df_weights = pd.DataFrame(data=weights_subset)
    # df_backbone = pd.merge(df_weights, ingr, left_on=0,
    #                       right_on='ingredient name').drop('ingredient name',
    #                                                        axis=1)
    ...
    for item in all_ingredients:
        recipe_count[item] = recipe_count['ingredients'].apply(
            lambda x: item in x)
    ...
    df_backbone = pd.merge(df_weights, ingr_count, left_on=0,
                           right_on='ingredient name').drop('ingredient name',
                                                            axis=1)
