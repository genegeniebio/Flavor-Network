'''Clean up recipes from yummly, to produce ingredient lists and flavor lists,
and normalize flavor profile using tfidf technique
Pickle the dataframe after clean up as 'yummly_clean.pkl'
Pickle the dataframe projected into the ingredient space in the flavor network
as 'yummly_ingr.pkl'
'''
import itertools
import re
...
def split_ingr(x):
    ...
    lst = x.strip('[]').split(',')
    cleanlist = [' '.join(wnl.lemmatize(word.lower())
...
def remove_word(word):
    alist = ['low fat', 'reduced fat', 'fat free', 'fatfree', 'nonfat',
             'gluten free', 'free range', 'reduced sodium', 'salt free',
             'sodium free', 'low sodium', 'sweetened', 'unsweetened', 'large',
             'extra large', 'oz ']
    for item in alist:
        word = word.replace(item, '')
    return word

# match ingredients in yummly recipes to ingredients in graph; filter out
# those with >3 missing matches
...
def cleanup_ingredients(ingr, df, col):
    ...
    df2['match ingredients'] = df2[col].apply(
        lambda x: ingr_replace(x, df_dic))
    df2['len_match'] = df2['match ingredients'].apply(lambda x: len(x))
    # remove entries with less match ingredients or no matching
    # ingr_ingredients
...
# Count missing ingredients after matching; salt, sugar, water and oil are
# not in the flavor network and thus don't count as missing
...
# After making dictionary to map ingredients from yummly recipes to
# ingredients in the flavor network, this is to replace ingredients in
...
# using flavor network to project recipes from ingredient matrix to flavor
# matrix
...
def flavor_profile(df, ingr, comp, ingr_comp):
    ...
    ingr_pivot = pd.crosstab(
        ingr_total['ingredient name'], ingr_total['compound id'])
...
if __name__ == '__main__':
    ...
    yum['cuisine'] = yum['cuisine'].apply(
        lambda x: cuisine_dic[x] if x in cuisine_dic else x)
    ...
    yum['clean ingredients'] = yum['ingredients'].apply(
        lambda x: split_ingr(x))
    yum['clean ingredients'] = yum['clean ingredients'].apply(
        lambda x: [remove_word(word) for word in x])
    ...
    ingr['space ingredients'] = ingr['ingredient name'].apply(
    ...
    yum_ingr, yum_X = cleanup_ingredients(
        ingr_ingredients, yum, 'clean ingredients')
