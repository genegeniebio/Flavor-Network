...
# %matplotlib inline
def finddishes(idx, yum_ingr2, yum_cos, similar_cuisine=False):
    '''this function prints out dishes similar to the query, either including
    or exclusing dishes from its own cuisine, based on similar_cuisine.'''
    cuisine = yum_ingr2.iloc[idx]['cuisine']
    print('Dishes similar to', yum_ingr2.ix[idx, 'recipeName'],
          '(' + yum_ingr2.ix[idx, 'cuisine'] + ')')
    ...
    for i in submatch.index:
        print(submatch.ix[i, 'recipeName'],
              '(' + submatch.ix[i, 'cuisine'] + ')', '(ID:' + str(i) + ')')


def plot_similardishes(idx, xlim, yum_ingr2, yum_cos):
    ...
    ax.set_title(yum_ingr2.ix[idx, 'recipeName'] +
                 '(' + yum_ingr2.ix[idx, 'cuisine'] + ')', fontsize=18)
...
def main():
    ...
    idx = 3900
    xlim = [0.91, 1.0]
    plot_similardishes(idx, xlim, yum_ingr2, yum_cos)
