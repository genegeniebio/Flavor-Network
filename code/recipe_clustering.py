...
def tsne_cluster_cuisine(df, sublist):
    ...
    for i, cuisine in enumerate(sublist):
        plt.scatter(tsne[lenlist[i]:lenlist[i + 1], 0],
                    tsne[lenlist[i]:lenlist[i + 1], 1], c=palette[i],
                    label=sublist[i])
    plt.legend()


def plot_bokeh(df, sublist, filename):
    '''interactive plot with boken; set up for four categories, with color
    palette; pass in df for either ingredient or flavor.'''
    lenlist = [0]
    df_sub = df[df['cuisine'] == sublist[0]]
    lenlist.append(df_sub.shape[0])
    for cuisine in sublist[1:]:
