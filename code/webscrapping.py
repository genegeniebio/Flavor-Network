...
def initial_df(cykey, scuisine1, scourse, sresults):
    ...
    '''
    r = (requests.get('http://api.yummly.com/v1/api/recipes?' +
                      cykey + scuisine1 + scourse + sresults)).json()
...
def grow_df(cuisinedic, cykey, scourse, sresults, yum):
    '''request recipes for all keys in cuisinedic and screen cuisine labels by
    values in cuisinedic.
    Merge new recipes with exisiting dataframe
    '''
    for cs in cuisinedic:
        scuisine = '&allowedCuisine[]=cuisine^cuisine-' + cs
        r = (requests.get('http://api.yummly.com/v1/api/recipes?' +
                          cykey + scuisine + scourse + sresults)).json()
        print scuisine
